...
const TEAMS: TeamSeed[] = [
  ...
  },
  {
    ...
    name: "BBL Esports",
    tag: "BBL",
    logo: img("65b8ccef5e273"),
    socials: {
      website: "https://bblesports.com/",
      twitter: "https://x.com/BBL_Esports",
      instagram: "https://instagram.com/bblespor",
    },
    group: "omega",
    seed: 3,
    players: [
...
const GROUP_MATCHES: [string, string, number, number, number][] = [
  // Poule Alpha
  ["vlr-fut", "vlr-tl", 1, 2, 5],
  ["vlr-fut", "vlr-th", 2, 0, 8],
  ["vlr-fut", "vlr-gm", 2, 1, 12],
  ["vlr-tl", "vlr-th", 2, 0, 15],
  ["vlr-tl", "vlr-gm", 1, 2, 19],
  ["vlr-th", "vlr-gm", 2, 1, 22],
  // Poule Omega
  ["vlr-fnc", "vlr-ef", 2, 0, 6],
  ["vlr-fnc", "vlr-vit", 2, 1, 9],
  ["vlr-fnc", "vlr-bbl", 2, 0, 13],
  ["vlr-ef", "vlr-vit", 1, 2, 16],
  ["vlr-ef", "vlr-bbl", 1, 2, 20],
  ["vlr-vit", "vlr-bbl", 2, 0, 23],
];
...
async function main() {
  ...
  for (const t of TEAMS) {
    await db.team.create({
      data: {
        ...
        name: t.name,
        tag: t.tag,
        logo: await mirror(t.logo, "teams", t.id),
        socials: t.socials,
        region: "EMEA",
        status: "ACTIVE",
      },
  ...
  console.log(
    `OK - tournoi ${TID} importé : ${TEAMS.length} équipes, ${GROUP_MATCHES.length} matchs de poule, ${BRACKET_MATCHES.length} playoffs.`
  );
}
