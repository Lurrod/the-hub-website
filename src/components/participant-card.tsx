...
export default function ParticipantCard({ p }: { p: P }) {
  return (
    <div className="card group relative flex h-40 w-40 flex-col items-center p-3 text-center">
      <div className="relative flex w-full flex-1 items-center justify-center">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-0.5 rounded bg-[var(--card)]/95 opacity-0 backdrop-blur-sm transition-opacity duration-150 group-hover:opacity-100">
          {p.players.length > 0 ? (
            p.players.map((pl) => (
              <Link
                key={pl.id}
