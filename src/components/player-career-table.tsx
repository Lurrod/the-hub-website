          ...
          {stints.map((s) => {
            ...
            return (
              ...
              >
                ...
                <td className={`${TD} text-center font-semibold text-white`}>{monthYear(s.joinDate)}</td>
                <td className={`${TD} text-center ${current ? "text-[var(--accent)]" : "text-[var(--text-muted)]"}`}>
                ...
                <td className={`${TD} stat text-center`}>
                  {s.winRate != null ? (
                    <span className={s.winRate >= 50 ? "text-white" : "text-[var(--text-muted)]"}>{s.winRate}%</span>
                  ) : (
                ...
                <td className={TD}>
                  {s.topAgents.length > 0 ? (
                    <div className="flex items-center justify-center gap-1">
                      ...
                      {s.extraAgents > 0 && (
                        <span className="stat ml-0.5 text-xs text-[var(--text-muted)]">+{s.extraAgents}</span>
                      )}
                    </div>
                  ) : (
