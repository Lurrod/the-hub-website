...
export default async function TournamentParticipantsPage({
  ...
  return (
    <main className="mx-auto max-w-2xl px-4 py-10">
      <h1 className="mb-6 text-2xl font-bold text-white">Inscrits · {tournament.name}</h1>
      ...
      {error && (
        <p className="mb-4 rounded border border-[var(--destructive)] bg-[var(--destructive-soft)] px-3 py-2 text-sm text-[var(--destructive)]">
          {error === "seedtaken"
            ? "Ce seed est déjà attribué à une autre équipe de ce tournoi."
            : "Données invalides : choisis une équipe et, si besoin, un seed valide."}
        </p>
      )}
