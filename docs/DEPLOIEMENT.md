...
**Masquage de version d'Apache** (une fois). `ServerTokens` n'est admis qu'en
contexte server, pas dans un VirtualHost : il se pose globalement, sinon
l'en-tête `Server` et les pages d'erreur annoncent la version exacte
(`Apache/2.4.58 (Ubuntu)`), un renseignement gratuit pour qui cherche une CVE.

```bash
# security.conf est fourni par Ubuntu et déjà activé.
...
sudo apache2ctl configtest && sudo systemctl reload apache2
# Vérifier : `curl -sI https://the-hub-vrc.fr | grep -i server` doit afficher « Server: Apache ».
```

`deploy/apache.conf` répète `ServerSignature Off` dans chaque VirtualHost et
réserve `/api/premier/sync` à la boucle locale (`Require local`) — cette route
n'est appelée que par la crontab du serveur.
